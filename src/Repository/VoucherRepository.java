...
public class VoucherRepository {
    ...
    public boolean add(Voucher vc){
        ...
        try {
            ...
            ps.setString(1, vc.getTenVocher());
...
public int Delete(Voucher vc) {
    try {
        ...
        p.setString(1, vc.getTenVocher()); // Đặt đúng kiểu dữ liệu
   ...
   public void EditByName(Voucher vc) {
    ...
    try (Connection c = DBConnect.getConnection();
        ...
        ps.setString(6, vc.getTenVocher());

        int rowsAffected = ps.executeUpdate();
        if (rowsAffected == 0) {
            System.out.println("Không tìm thấy voucher để cập nhật theo tên: " + vc.getTenVocher());
        }
    } catch (Exception e) {
        e.printStackTrace();
    }
